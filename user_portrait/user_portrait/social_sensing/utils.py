...
def get_text_detail(task_name, ts, text_type, user, order, size=100):
    ...
    if int(text_type) == 0: # 热门原创微博
        ...
        results = get_positive_weibo_detail(ts, user, task_name, size, "sentiment", "1")

    elif int(text_type) == 5: # 中性微博
        results = get_positive_weibo_detail(ts, user, task_name, size, "sentiment", "0")

    elif int(text_type) == 6: # 消极微博
        results = get_positive_weibo_detail(ts, user, task_name, size, "sentiment", ["2", "3", "4", "5", "6"])
