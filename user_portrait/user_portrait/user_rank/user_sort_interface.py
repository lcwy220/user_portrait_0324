...
from Offline_task import add_task , search_task



#keyword splite by ','   and the 关键词和hashtag改为离线任务
def user_sort_interface(username  = None , time = 1 , sort_norm = 'imp' , sort_scope = 'in_nolimit' , arg = None , start_time = '2013-09-01' , end_time = '2013-09-07'):
    ...
    try:
        ...
        if sort_scope == 'all_nolimit' :
            ...
            search_key = add_task(user_name = username ,key = arg ,type= "keyword" ,range = "all")
            time = 1
            if during > 3:
                time = 7
            elif during > 16:
                time = 30
            return key_words_search(username,'flow_text_',during,start_time,arg,"all",search_key,sort_norm,time);

        elif sort_scope == "in_limit_keyword":
            during = ( datetime2ts(end_time) - datetime2ts(start_time) ) / DAY
            search_key = add_task(user_name = username ,key = arg ,type= "keyword" ,range = "in")
            time = 1
            if during > 3:
                time = 7
            elif during > 16:
                time = 30 
            return key_words_search(username,'flow_text_',during,start_time,arg,"in",search_key,sort_norm,time);       
        else :
...
def search_user_offline_task(username):
    return search_task(user_name)
...
if __name__ == "__main__":
    ...
    import json
    print json.dumps( user_sort_interface( 'kanon', time=1,sort_norm="fans",sort_scope="all_limit_keyword" ,arg="heel") )
