...
from global_utils import COPY_USER_PORTRAIT_ACTIVENESS, COPY_USER_PORTRAIT_ACTIVENESS_TYPE
...
def compute_week(item, now_ts):
    ...
    for i in range(7):
        ...
        week_list.append("activeness_"+date_string)
...
def compute_month(item, now_ts):
    ...
    for i in range(30):
        date_string = str(now_ts - i*DAY)
        month_list.append("activeness_"+date_string)
...
def co_search(add_info, update_bci_key, former_bci_key, now_ts):
    ...
    evaluate_history_results = es_user_portrait.mget(index=COPY_USER_PORTRAIT_ACTIVENESS, doc_type=COPY_USER_PORTRAIT_ACTIVENESS_TYPE,body={'ids':uid_list})['docs']
    iter_count = 0
    bulk_action = []
    for uid in uid_list:
        ...
        if item['found']:
            ...
            user_history_item['activeness_day_change'] = user_history_item[update_bci_key] - user_history_item.get(former_bci_key, 0)
            user_history_item['activeness_week_change'] = user_history_item[update_bci_key] - user_history_item.get('activeness_week_ave', 0)
            user_history_item['activeness_month_change'] = user_history_item[update_bci_key] - user_history_item.get('activeness_month_ave', 0)
            user_history_item['activeness_week_ave'], user_history_item['activeness_week_var'], user_history_item['activeness_week_sum'] = compute_week(user_history_item, now_ts)
            ...
            user_history_item.update(add_info[uid])
            user_history_item['activeness_day_change'] = user_history_item[update_bci_key]
            user_history_item['activeness_week_change'] = user_history_item[update_bci_key]
            user_history_item['activeness_month_change'] = user_history_item[update_bci_key]
            user_history_item['activeness_week_ave'], user_history_item['activeness_week_var'], user_history_item['activeness_week_sum'] = compute_week(user_history_item, now_ts)
            user_history_item['activeness_month_ave'], user_history_item['activeness_month_var'], user_history_item['activeness_month_sum'] = compute_month(user_history_item, now_ts)
        iter_count += 1
    ...
    if bulk_action:
        es_cluster.bulk(bulk_action, index=COPY_USER_PORTRAIT_ACTIVENESS, doc_type=COPY_USER_PORTRAIT_ACTIVENESS_TYPE,timeout=600)
    print iter_count
...
def main():
    ...
    max_influence = get_max_index('activeness')
    update_bci_key = "activeness_" + ts # 更新的键
    del_month = str(datetime2ts(ts2datetime(now_ts - MONTH)))
    del_bci_key = "activeness_" + del_month

    former_ts = now_ts - DAY
    former_date = str(datetime2ts(ts2datetime(former_ts)))
    former_bci_key = "activeness_" + former_date
