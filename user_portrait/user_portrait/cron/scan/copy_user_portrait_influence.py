...
from global_utils import COPY_USER_PORTRAIT_INFLUENCE, COPY_USER_PORTRAIT_INFLUENCE_TYPE
from parameter import RUN_TYPE, RUN_TEST_TIME, LOW_INFLUENCE_THRESHOULD

def compute_week(item, now_ts):
    week_list = []
    for i in range(7):
        date_string = str(now_ts - i*DAY)
        week_list.append("bci_"+date_string)
...
def compute_month(item, now_ts):
    ...
    for i in range(30):
        date_string = str(now_ts - i*DAY)
        month_list.append("bci_"+date_string)
