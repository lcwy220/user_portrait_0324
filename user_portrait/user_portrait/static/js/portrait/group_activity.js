...
function bdGEO(geolist,from_city,end_city,dealt_data){
    for(var i=0;i<geolist.length;i++){
		geoname = geolist[i];
		geocodeSearch(geoname,geolist,from_city,end_city,dealt_data);
	}
}
function geocodeSearch(geoname,geolist,from_city,end_city,dealt_data){

    setTimeout(drawroute(from_city,end_city,dealt_data), 400);
...
function drawroute(from_city,end_city,dealt_data){
		var route = [];
        for(var i=0;i<from_city.length;i++){
	       route.push({'name':from_city[i]}, {'name':end_city[i],'value':dealt_data[0][i]});
        }
...
function Draw_moving(from_city, end_city, dealt_data){
   //console.log(dealt_data);
   /*
   var geolist = [];
   for(var i=0;i<from_city.length;i++){
	   geolist.push(from_city[i]);
   }
   for(var i=0;i<end_city.length;i++){
	   geolist.push(end_city[i]);
   }
   bdGEO(geolist,from_city,end_city,dealt_data);  
   */   
